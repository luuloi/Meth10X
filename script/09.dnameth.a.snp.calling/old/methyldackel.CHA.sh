...
LOGFILE="${output}/${sample}.methyldackel.meth.calling.CHA.log"

echo `date`" - CHA strands" > $LOGFILE
echo -e "#chr\tposition\tstrand\t${sample}.C\t${sample}.cov" > "${output}/${sample}.MD.strands_CHA.tsv"
cmd="""
bedtools intersect -a ${GENOME/.fa/.CHA.strands.bed} -b ${output}/${sample}.MD.strands_CHH.bedGraph -loj| awk '{OFS=\"\t\"}{if(\$11==-1){\$11=\$12=0}; print \$1,\$3,\$6,\$11,\$11+\$12}' >> ${output}/${sample}.MD.strands_CHA.tsv
"""
echo $cmd >> "$LOGFILE"; eval $cmd 2>> "$LOGFILE"; echo -e `date`" Finished - CHA strands\n" >> $LOGFILE

echo "*** Zip output file"  >> $LOGFILE
cmd="""
gzip ${output}/${sample}.MD.strands_CHA.tsv;
"""
