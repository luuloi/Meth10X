...
mkdir -p $OUTPUT
LOGFILE="$OUTPUT/${sample}.compute.distribution.100kb.log"
...
bedtools groupby -i - -g 1,2,3 -c 7 -o mean| awk '{ printf \"%.4f\n\", \$4}'| sort -T $OUTPUT| uniq -c| awk -v sample=$sample '{OFS=\"\t\"}{print sample,\$2,\$1}' > "$OUTPUT/${sample}.distribution.100kb.tsv"
""" 
