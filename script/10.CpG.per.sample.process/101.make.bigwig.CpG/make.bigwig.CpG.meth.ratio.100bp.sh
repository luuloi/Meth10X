...
LOGFILE="$OUTPUT/${sample}.make.bigwig.CpG.meth.ratio.100bp.log"
...
echo " - 100bp"  >> "$LOGFILE"
cmd="""
bedtools intersect -a <(bedtools makewindows -g $GSIZE -w 100) -b "${INPUT/.cov.bw/.meth.ratio.bedGraph}" -loj|
awk '{OFS=\"\t\"}{if(\$7==\".\"){\$7=-1}; print \$1,\$2,\$3,\$7}'|
bedtools groupby -g 1,2,3 -c 4 -o collapse|
awk '{OFS=\"\t\"}{s=0; c=0; split(\$4,a,\",\"); for(i=1;i<=length(a);i++){if(a[i]>=0){s=s+a[i];c=c+1}}; if(c>0){print \$1,\$2,\$3,s/c}else{print \$1,\$2,\$3,-1}}' > "$OUTPUT/${sample}.smoothed.100bp.bedGraph"
"""
echo $cmd >> "$LOGFILE"; eval $cmd 2>> "$LOGFILE"
cmd=""" bedGraphToBigWig "$OUTPUT/${sample}.smoothed.100bp.bedGraph" $GSIZE "$OUTPUT/${sample}.100bp.bw" """
echo $cmd >> "$LOGFILE"; eval $cmd 2>> "$LOGFILE"

# clean
echo " - clean up"  >> "$LOGFILE"
cmd=""" 
rm "$OUTPUT/${sample}.smoothed.100bp.bedGraph"
"""
echo $cmd >> "$LOGFILE"; eval $cmd 2>> "$LOGFILE"
