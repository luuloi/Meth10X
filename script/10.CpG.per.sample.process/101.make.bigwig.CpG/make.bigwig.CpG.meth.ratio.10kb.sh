...
LOGFILE="$OUTPUT/${sample}.make.bigwig.CpG.meth.ratio.10kb.log"
...
echo " - 10kb"  >> "$LOGFILE"
cmd="""
bedtools intersect -a <(bedtools makewindows -g $GSIZE -w 10000) -b "${INPUT/.cov.bw/.meth.ratio.bedGraph}" -loj|
...
cmd=""" 
rm "$OUTPUT/${sample}.smoothed.10kb.bedGraph"
"""
echo $cmd >> "$LOGFILE"; eval $cmd 2>> "$LOGFILE"
