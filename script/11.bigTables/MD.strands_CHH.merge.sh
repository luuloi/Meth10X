...
# $2=bigTables
output=$2
LOGFILE="$output/MD.strands_CHH.merge.log"
...
echo `date`" *** Merge MD.strands_CHH samples into bigTable" > $LOGFILE

echo "Sort and eliminate sample duplication" >> $LOGFILE
cmd="input_unique=\$(echo $input| awk '{for(i=1;i<=NF;i++){if(\$i ~ /MD.strands_CHH.merge/) print \$i}}'| sort -u)"
...
cmd="pigz -f $output/bigTable.CHH.strand.tsv"
...
# zcat called/PrEC//PrEC.MD.strands_CHH.tsv.gz| head
