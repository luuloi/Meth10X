...
# $2=bigTables
output=$2
LOGFILE="$output/MD.strands_lambda.merge.log"
...
echo `date`" *** Merge MD.strands_lambda samples into bigTable" > $LOGFILE

echo "Sort and eliminate sample duplication" >> $LOGFILE
cmd="input_unique=\$(echo $input| awk '{for(i=1;i<=NF;i++){if(\$i ~ /MD.strands_lambda.merge/) print \$i}}'| sort -u)"
