...
# $2=bigTables
output=$2
LOGFILE="$output/MD.strands_CHC.merge.log"
...
echo `date`" *** Merge MD.strands_CHC samples into bigTable" > $LOGFILE

echo "Sort and eliminate sample duplication" >> $LOGFILE
cmd="input_unique=\$(echo $input| awk '{for(i=1;i<=NF;i++){if(\$i ~ /MD.strands_CHC.merge/) print \$i}}'| sort -u)"
...
cmd="$cmd > $output/bigTable.CHC.strand.tsv"
echo $cmd >> "$LOGFILE"; eval $cmd 2>> "$LOGFILE"; echo -e `date`" Finished merge\n" >> $LOGFILE

cmd="gzip $output/bigTable.CHC.strand.tsv"
echo $cmd >> "$LOGFILE"; eval $cmd 2>> "$LOGFILE"; echo -e `date`" Finished zip\n" >> $LOGFILE

echo `date`" Finished merge MD.strands_CHC samples into bigTable" >> $LOGFILE
