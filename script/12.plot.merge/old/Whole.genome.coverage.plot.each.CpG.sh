...
LOGFILE="$output/Whole.genome.coverage.plot.each.CpG.log"

echo "Whole.genome.coverage.plot.each.CpG.sh" > "$LOGFILE"
echo `date`" - Started processing $1 on $HOSTNAME" >> "$LOGFILE"
Rscript "$BASEDIR/Whole.genome.coverage.plot.each.CpG.r" "${input/.gz/}" "$output" 2>> "$LOGFILE"
